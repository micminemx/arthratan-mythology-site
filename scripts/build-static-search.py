...
# Characters/concepts should be discoverable before raw-page hits, matching the
# ranking contract while keeping every record in the first HTML response.
...
for i, r in enumerate(records, 1):
  ...
  <div class="result-meta"><span class="type">{esc(r['type'])}</span><span class="source-id">{esc(r.get('source_id',''))}</span></div>
  <h2><a href="{esc(r['route'])}">{esc(r['title'])}</a></h2>
  {alias_html}
  <p class="snippet">{esc(r.get('snippet',''))}</p>
  <p class="provenance"><span>Discovery excerpt — not a replacement for source canon.</span> <a href="{esc(r.get('source_path',''))}">Source/data record</a></p>
</article>''')
...
<meta name="description" content="Complete crawlable Arthratan Mythology search corpus across characters, Zubaida transmissions, Divine v144, Hypergendered Logic, living canon, concepts and provenance. JavaScript is not required.">
...
<body>
<header>
<nav aria-label="Codex discovery"><a href="/">Sanctuary</a><a href="/characters/">Characters</a><a href="/zubaida/">Zubaida</a><a href="/divine/">Divine v144</a><a href="/hgl/">HGL</a><a href="/crawl/">Crawl index</a></nav>
<p class="eyebrow">Cross-source discovery · source-preserving</p>
<h1>Search the Arthitean Codex</h1>
<p class="lede">This page contains the complete unified search corpus in its <strong>initial HTML response</strong>. JavaScript is optional: it only filters and highlights what is already present. AI agents, text browsers and crawlers can read and follow every result without executing scripts.</p>
<div class="notice"><strong>{len(records)} searchable records.</strong> {count_summary}. Source snippets are discovery aids; canonical source material remains separately retrievable through each result's source/data link.</div>
<form class="tools" action="/search/" method="get" role="search">
<label><span class="visually-hidden">Search terms</span><input id="q" name="q" type="search" autocomplete="off" placeholder="Try Dyvane Redalious, Hypernegative Rewrite, Lydrextent, Inzra…"></label>
...
<footer>Generated from SEARCH-001 corpus snapshot {esc(meta.get('generated_from_commit','unknown'))}. Search is progressive enhancement: content access never depends on JavaScript.</footer>
<script>
(()=>{{
...
plain = [
    ...
    '# Format: title | type | route | aliases | source/data path',
...
# DeepSeek regression fixture: the static first-response HTML must itself contain
# Dyvane and a normal character URL before any JavaScript executes.
