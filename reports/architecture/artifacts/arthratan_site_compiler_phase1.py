...
Performs exhaustive static analysis across the entire website corpus:
1. File & canonical URL discovery
2. Comprehensive link graph construction (resolving relative paths, root-relative, absolute domain links, hash fragments, static assets)
3. Broken link & broken anchor detection
4. Home click depth BFS distribution and exact moments (mean, median, mode, variance, SD, skewness, kurtosis, P50, P75, P90, P95, P99, max, Gini)
5. Degree distribution & Gini coefficient
6. Graph connectivity (SCC, WCC, diameter, average shortest path length)
7. Centrality measures (PageRank with alpha=0.85, Brandes Betweenness Centrality)
8. Hub-removal topological robustness testing (top 1, 3, 5, 10 hubs removed)
9. Semantic reciprocity & section relationship matrix
10. Sitemap & search coverage validation
11. Worst pages ranking by architectural penalty
12. Regression comparison against baseline metrics
13. Diagnostic visual chart generation (.png)
"""
...
def resolve_link(source_rel, href, site_root, all_pages_set, file_anchors_map):
    """
    Resolves an href from source_rel to a target canonical rel_path or static asset.
    Returns: (target_rel, fragment, is_external, is_broken, is_anchor_broken, is_html_page)
    """
    ...
    for c in candidates:
        ...
        if os.path.isfile(full):
            resolved = norm_c
            is_html_page = (norm_c in all_pages_set or norm_c.endswith('.html'))
            break

    if resolved is None:
        return (None, fragment, False, True, False, False)
        
    is_anchor_broken = False
    if fragment and resolved in file_anchors_map:
        valid_anchors = file_anchors_map[resolved]
        if fragment not in valid_anchors:
            is_anchor_broken = True
            
    return (resolved, fragment, False, False, is_anchor_broken, is_html_page)
...
def parse_site(site_root):
    ...
    for rel_path, _ in html_files:
        ...
        for a_tag in soup.find_all('a', href=True):
            raw_href = a_tag['href']
            tgt, frag, is_ext, is_broken, is_anchor_broken, is_html = resolve_link(
                rel_path, raw_href, site_root, all_pages_set, file_anchors
            )
            ...
            if is_broken:
                ...
                if is_html and tgt in all_pages_set and tgt != rel_path:
...
def audit_sitemap(site_root, all_pages_set):
    ...
    for loc in locs:
        ...
        if p in all_pages_set:
            sitemap_pages.add(p)
        elif (p + '/index.html').replace('//', '/') in all_pages_set:
            sitemap_pages.add((p + '/index.html').replace('//', '/'))
        elif (p.rstrip('/') + '/index.html').replace('//', '/') in all_pages_set:
            sitemap_pages.add((p.rstrip('/') + '/index.html').replace('//', '/'))
        elif (p + '.html') in all_pages_set:
    ...
    missing_from_sitemap = sorted(list(all_pages_set - sitemap_pages))
...
def generate_visual_diagnostics(report_dir, depths, in_degrees, out_degrees, pagerank, betweenness, hub_data):
    ...
    if d_vals:
        ...
        plt.title('Home Click Depth Distribution', fontsize=14, fontweight='bold')
    ...
    plt.title('Page Degree Distributions', fontsize=14, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(description="Arthitean Site Architecture Compiler")
    parser.add_argument("site_root", default=".", help="Path to site root")
    parser.add_argument("--report-dir", default="reports/architecture", help="Directory to store reports")
    ...
    print(f"[COMPILER] Initializing site compilation for: {site_root}")
    ...
    print(f"[COMPILER] Discovered {len(all_pages)} HTML pages across site.")
    ...
    sitemap_audit = audit_sitemap(site_root, data["all_pages_set"])
    ...
    # In net_metrics, add broken counts for clean regression tracking
    ...
    nav_metrics_out = {
        "total_html_files": len(all_pages),
    ...
    md_report_lines = [
        ...
        f"**Compile Status**: `{'PASS' if compile_report_json['status'] == 'PASS' else 'FAIL — ARCHITECTURAL ISSUES DETECTED'}`",
        f"**Total Pages Processed**: {len(all_pages)} | **Total Directed Links**: {G.number_of_edges()}",
        ...
        "| :--- | :---: | :---: | :---: |",
        f"| Home Reachable Pages | {net_metrics['home_reachable_count']} / {len(all_pages)} ({round(net_metrics['home_reachable_count']*100/len(all_pages), 2)}%) | 100% | `{'PASS' if len(unreachable) == 0 else 'FAIL'}` |",
        f"| Unreachable Pages | {len(unreachable)} | 0 | `{'PASS' if len(unreachable) == 0 else 'FAIL'}` |",
        f"| Orphan Pages (In-Degree = 0) | {net_metrics['orphan_count']} | 0 | `{'PASS' if net_metrics['orphan_count'] == 0 else 'FAIL'}` |",
        f"| Dead-End Pages (Out-Degree = 0) | {net_metrics['dead_end_count']} | 0 | `{'PASS' if net_metrics['dead_end_count'] == 0 else 'WARNING'}` |",
        f"| Broken Internal Links | {len(broken_links)} | 0 | `{'PASS' if len(broken_links) == 0 else 'FAIL'}` |",
        f"| Broken Anchor References | {len(broken_anchors)} | 0 | `{'PASS' if len(broken_anchors) == 0 else 'WARNING'}` |",
        f"| Unrendered Template Pages | {nav_metrics_out['unrendered_templates_count']} | 0 | `{'PASS' if nav_metrics_out['unrendered_templates_count'] == 0 else 'FAIL'}` |",
        ...
        f"| Sitemap Coverage | {sitemap_audit['valid_pages_in_sitemap']} / {len(all_pages)} ({sitemap_audit['missing_count']} missing) | 100% | `{'PASS' if sitemap_audit['missing_count'] == 0 else 'FAIL'}` |\n",
        "## Exact Depth Moments & Quantiles",
        "| Statistic | Value |",
        "| :--- | :---: |",
        f"| Count | {net_metrics['depth_moments']['count']} |",
        f"| Mean | {net_metrics['depth_moments']['mean']} |",
        f"| Median | {net_metrics['depth_moments']['median']} |",
        f"| Mode | {net_metrics['depth_moments']['mode']} |",
        f"| Variance | {net_metrics['depth_moments']['variance']} |",
        f"| Standard Deviation | {net_metrics['depth_moments']['std_dev']} |",
        f"| Skewness | {net_metrics['depth_moments']['skewness']} |",
        f"| Excess Kurtosis | {net_metrics['depth_moments']['kurtosis_excess']} |",
        f"| P50 / P75 / P90 | {net_metrics['depth_moments']['p50']} / {net_metrics['depth_moments']['p75']} / {net_metrics['depth_moments']['p90']} |",
        f"| P95 / P99 / Max | {net_metrics['depth_moments']['p95']} / {net_metrics['depth_moments']['p99']} / {net_metrics['depth_moments']['max']} |",
        f"| Depth Gini | {net_metrics['depth_moments']['gini']} |\n",
    ...
    md_report_lines.append("| Rank | Page | Penalty Score | Depth | In-Deg | Out-Deg | Issues |")
    md_report_lines.append("| :---: | :--- | :---: | :---: | :---: | :---: | :--- |")
    for idx, wp in enumerate(worst_pages[:10], 1):
        md_report_lines.append(f"| {idx} | `{wp['page']}` | **{wp['score']}** | {wp['depth']} | {wp['in_degree']} | {wp['out_degree']} | {'; '.join(wp['reasons'])} |")
    ...
    if critical_errors > 0:
        ...
        print("[COMPILER] PASS: 0 critical architectural errors detected. Site architecture healthy.")
        sys.exit(0)
